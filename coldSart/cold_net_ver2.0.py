...
log_writer = logging.FileHandler('cold_net_new.log', 'a')
...
class cold_model():
    def __init__(self, act_num, country_num, director_num,
        ...
        logger.debug(printformat % (dim, act_num, country_num, director_num, genre_num))
    ...
    def backward(self, user, eui, actor, country, director, genre, maxgrad=3):
        ...
        for t in range(self.dim):
            ...
            mid_act = self.ud_actor_v[t] * (1 - self.actor_f[t])
            mid_country = self.ud_country_v[t] * (1 - self.country_f[t])
            mid_director = self.ud_director_v[t] * (1 - self.director_f[t])
            mid_genre = self.ud_genre_v[t] * (1 - self.genre_f[t])
